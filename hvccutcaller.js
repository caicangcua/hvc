...
function windowRESIZE() {
    ...
    nodataMsg.text('W: ' + $(window).width() + ' - H: ' + $(window).height());
...
function XuLy_CancelCall(CANCELS) {
    ...
    for (var i = 0; i < CANCELS.length; i++) {
        if (_MEMORY.hasOwnProperty(CANCELS[i])) {
            ...
            var spaceR = dataR.next();
            if (spaceR && spaceR.hasClass('spaceR')) { spaceR.remove(); };
            dataR.remove();
    ...
    if (isUI) {
        ...
        if (isExistR < 5) {
            ...
            tb3R(CALLS); // fill lại table3 cho đủ 5 rows
        };
        if (isExistR + CALLS.length <= 1) {
            _table3.find('tr.spaceR').remove();
        };
        PagingUI(isExistR);
