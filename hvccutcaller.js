...
function ModCall_TB3(CALL) {
    ...
    if (TR.length > 0) {
        ...
        TR.find('.partno').text(CALL.FF_PartNo.substr(CALL.FF_PartNo.length - 10));
    ...
    for (var i = 0; i < calls.length; i++) {
        ...
        if (isExistR >= limitTable3R) {
            ...
            var newTR = $(((i == 0 && isExistR > 0) ? spaceR : '') + '<tr id="' + CALL.MsgID + '" class="dataR">' +
                ...
                '</th>' +
                '<td>' +
                    '<div class="taskpn"><span class="taskid">' + CALL.FF_TaskID + '</span><span class="partno">' + CALL.FF_PartNo.substr(CALL.FF_PartNo.length - 10) + '</span></div>' +
